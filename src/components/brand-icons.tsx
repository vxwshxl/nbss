 ...
 *
 * `lucide-react` is the app's only icon set, and it deliberately ships no brand
 * marks — so these live here rather than as a second icon dependency. They are
 * filled paths, not strokes, because that is how each owner draws its mark; do
 * not restyle them.
 *
 * Each one is paired with its owner's colour below rather than inheriting the
 * page's. A WhatsApp glyph in the brand green would be a different green, and a
 * mark in almost-the-right-colour stops being recognisable at a glance — which
 * is the entire reason for using someone else's mark in the first place.
 */

export type BrandKey = "WhatsApp" | "Facebook" | "Instagram" | "YouTube" | "X";
...
export function FacebookIcon({ className }: { className?: string }) {
  return (
    <svg
      ...
      className={cn("size-4", className)}
    >
      <path d="M24 12.073C24 5.405 18.627 0 12 0S0 5.405 0 12.073C0 18.1 4.388 23.094 10.125 24v-8.437H7.078v-3.49h3.047V9.413c0-3.025 1.792-4.696 4.533-4.696 1.313 0 2.686.236 2.686.236v2.971H15.83c-1.491 0-1.956.93-1.956 1.886v2.263h3.328l-.532 3.49h-2.796V24C19.612 23.094 24 18.1 24 12.073" />
    </svg>
  );
}
...
/**
 * A profile's glyph and its owner's colour, keyed by the `label` in
 * `content/site.social`.
 *
 * Keying on the label means a new profile is added to the site by adding one
 * line of content and one line here — and a profile whose label has no entry
 * still renders, as a neutral text chip, rather than throwing. Content should
